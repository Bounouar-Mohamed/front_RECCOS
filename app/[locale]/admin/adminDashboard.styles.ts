import { css } from '@/styled-system/css';

export const adminDashboardStyles = {
  container: css({
    paddingTop: { base: 'calc(5vh + 100px) !important', md: 'calc(5vh + 110px) !important' },

    minH: '100vh',
    width: '100%',
    bg: '#040404',
    color: '#f5f5f5',
    px: { base: '16px', md: '32px' },
    py: { base: '32px', md: '48px' },
  }),
  content: css({
    width: '100%',
    maxW: '1400px',
    mx: 'auto',
    display: 'flex',
    flexDirection: 'column',
    gap: '20px',
  }),
  hero: css({
    display: 'flex',
    flexDirection: { base: 'column', lg: 'row' },
    alignItems: { base: 'flex-start', lg: 'center' },
    justifyContent: 'space-between',
    gap: { base: '16px', lg: '24px' },
  }),
  heroTitle: css({
    fontSize: { base: '2rem', md: '2.8rem' },
    fontWeight: 600,
    letterSpacing: '-0.02em',
  }),
  heroSubtitle: css({
    fontSize: { base: '0.95rem', md: '1rem' },
    color: 'rgba(255,255,255,0.65)',
    maxW: '640px',
    lineHeight: 1.5,
  }),
  heroMeta: css({
    display: 'flex',
    alignItems: 'center',
    gap: '12px',
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.7)',
    flexWrap: 'wrap',
  }),
  heroActions: css({
    display: 'flex',
    alignItems: 'center',
    gap: '10px',
    flexWrap: 'wrap',
  }),
  heroHighlights: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(2, minmax(0, 1fr))', md: 'repeat(4, minmax(0, 1fr))' },
    gap: '12px',
  }),
  heroHighlightCard: css({
    borderRadius: '20px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(255,255,255,0.03)',
    padding: '16px',
    display: 'flex',
    flexDirection: 'column',
    gap: '6px',
  }),
  heroHighlightValue: css({
    fontSize: '1.6rem',
    fontWeight: 600,
  }),
  heroHighlightLabel: css({
    fontSize: '0.8rem',
    letterSpacing: '0.16em',
    textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.6)',
  }),
  primaryButton: css({
    borderRadius: '999px',
    border: 'none',
    background: 'linear-gradient(120deg, #ffffff 0%, #b3b3b3 100%)',
    color: '#050505',
    fontSize: '0.9rem',
    fontWeight: 600,
    paddingInline: '18px',
    paddingBlock: '10px',
    cursor: 'pointer',
    transition: 'transform 0.2s ease, box-shadow 0.2s ease',
    _hover: {
      transform: 'translateY(-1px)',
      boxShadow: '0 10px 24px rgba(255,255,255,0.18)',
    },
  }),
  ghostButton: css({
    borderRadius: '999px',
    border: '1px solid rgba(255,255,255,0.18)',
    background: 'transparent',
    color: '#ffffff',
    fontSize: '0.85rem',
    fontWeight: 500,
    paddingInline: '16px',
    paddingBlock: '9px',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease, color 0.2s ease',
    _hover: {
      borderColor: 'rgba(255,255,255,0.35)',
      color: '#fafafa',
    },
  }),
  statsGrid: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(2, minmax(0, 1fr))', md: 'repeat(4, minmax(0, 1fr))' },
    gap: '16px',
    width: '100%',
  }),
  statCard: css({
    borderRadius: '20px',
    padding: '16px',
    background: 'rgba(255,255,255,0.04)',
    border: '1px solid rgba(255,255,255,0.07)',
    display: 'flex',
    flexDirection: 'column',
    gap: '6px',
    minH: '110px',
  }),
  statLabel: css({
    fontSize: '0.75rem',
    textTransform: 'uppercase',
    letterSpacing: '0.2em',
    color: 'rgba(255,255,255,0.6)',
  }),
  statValue: css({
    fontSize: { base: '1.7rem', md: '2rem' },
    fontWeight: 600,
    letterSpacing: '-0.03em',
  }),
  sectionGroup: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '18px',
  }),
  quickNavGrid: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(1, minmax(0, 1fr))', md: 'repeat(2, minmax(0, 1fr))', lg: 'repeat(4, minmax(0, 1fr))' },
    gap: '14px',
    width: '100%',
  }),
  panelViewport: css({
    marginTop: '4px',
  }),
  panelShell: css({
    borderRadius: '28px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(0,0,0,0.35)',
    padding: { base: '16px', md: '22px' },
    minHeight: '360px',
  }),
  panelPlaceholder: css({
    minHeight: '240px',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    textAlign: 'center',
    color: 'rgba(255,255,255,0.6)',
    fontSize: '0.95rem',
  }),
  quickNavCard: css({
    borderRadius: '24px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(255,255,255,0.03)',
    padding: '18px',
    minH: '140px',
    display: 'flex',
    flexDirection: 'column',
    justifyContent: 'space-between',
    gap: '10px',
    color: '#f7f7f7',
    textAlign: 'left',
    textDecoration: 'none',
    transition: 'transform 0.2s ease, border-color 0.2s ease, box-shadow 0.2s ease',
    cursor: 'pointer',
    gridColumn: 'span 1',
    width: '100%',
    '&:hover': {
      transform: 'translateY(-2px)',
      borderColor: 'rgba(255,255,255,0.3)',
      boxShadow: '0 10px 25px rgba(0,0,0,0.35)',
    },
    '&[data-active="true"]': {
      borderColor: 'rgba(255,255,255,0.45)',
      boxShadow: '0 12px 32px rgba(0,0,0,0.45)',
      transform: 'translateY(-3px)',
    },
    '&[data-size="wide"]': {
      gridColumn: { base: 'span 1', lg: 'span 2' },
    },
    '&[data-size="tall"]': {
      gridRow: { base: 'span 1', lg: 'span 2' },
    },
    '&[data-accent="platinum"]': {
      background: 'linear-gradient(135deg, rgba(255,255,255,0.12), rgba(255,255,255,0.02))',
    },
    '&[data-accent="onyx"]': {
      background: 'linear-gradient(135deg, rgba(18,18,18,0.95), rgba(34,34,34,0.75))',
    },
    '&[data-accent="amber"]': {
      background: 'linear-gradient(135deg, rgba(255,194,111,0.2), rgba(255,153,0,0.05))',
    },
    '&[data-accent="azure"]': {
      background: 'linear-gradient(135deg, rgba(111,198,255,0.2), rgba(25,118,210,0.07))',
    },
  }),
  quickNavHeader: css({
    display: 'flex',
    alignItems: 'center',
    gap: '10px',
    fontSize: '1rem',
    fontWeight: 600,
  }),
  quickNavIcon: css({
    fontSize: '1.4rem',
  }),
  quickNavLabel: css({
    flex: 1,
    letterSpacing: '0.04em',
    textTransform: 'uppercase',
    fontSize: '0.85rem',
  }),
  quickNavGlyph: css({
    fontSize: '1rem',
    opacity: 0.7,
  }),
  quickNavDescription: css({
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.75)',
    lineHeight: 1.4,
  }),
  quickNavMetric: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '2px',
  }),
  quickNavMetricValue: css({
    fontSize: '1.4rem',
    fontWeight: 600,
  }),
  quickNavMetricLabel: css({
    fontSize: '0.75rem',
    textTransform: 'uppercase',
    letterSpacing: '0.16em',
    color: 'rgba(255,255,255,0.55)',
  }),
  statHelper: css({
    fontSize: '0.8rem',
    color: 'rgba(255,255,255,0.6)',
  }),
  sectionHeader: css({
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'flex-start',
    flexWrap: 'wrap',
    gap: '10px',
    marginBottom: '12px',
  }),
  sectionTitle: css({
    fontSize: '0.85rem',
    letterSpacing: '0.2em',
    textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.65)',
  }),
  sectionSubtitle: css({
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.6)',
    marginTop: '6px',
    maxW: '540px',
    lineHeight: 1.4,
  }),
  distributionCard: css({
    borderRadius: '22px',
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(255,255,255,0.06)',
    padding: '20px',
  }),
  distributionGrid: css({
    display: 'grid',
    gridTemplateColumns: { base: '1fr', md: 'repeat(3, minmax(0, 1fr))' },
    gap: '18px',
  }),
  distributionColumn: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '10px',
  }),
  distributionTitle: css({
    fontSize: '0.85rem',
    color: 'rgba(255,255,255,0.7)',
    letterSpacing: '0.08em',
  }),
  distributionPlaceholder: css({
    fontSize: '0.85rem',
    color: 'rgba(255,255,255,0.55)',
  }),
  distributionItem: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '6px',
  }),
  distributionMeta: css({
    display: 'flex',
    justifyContent: 'space-between',
    fontSize: '0.85rem',
    color: 'rgba(255,255,255,0.82)',
    gap: '12px',
  }),
  distributionPercentage: css({
    color: 'rgba(255,255,255,0.6)',
    fontSize: '0.8rem',
  }),
  distributionTrack: css({
    width: '100%',
    height: '6px',
    borderRadius: '999px',
    background: 'rgba(255,255,255,0.08)',
    overflow: 'hidden',
    '& span': {
      display: 'block',
      height: '100%',
      borderRadius: '999px',
      background: 'linear-gradient(120deg, #ffffff 0%, #b3b3b3 100%)',
      transition: 'width 0.35s ease',
    },
  }),
  performanceCard: css({
    borderRadius: '22px',
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(255,255,255,0.06)',
    padding: '20px',
  }),
  performanceGrid: css({
    display: 'grid',
    gridTemplateColumns: { base: '1fr', lg: 'repeat(3, minmax(0, 1fr))' },
    gap: '18px',
  }),
  performanceColumn: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
  }),
  performanceTitle: css({
    fontSize: '0.9rem',
    letterSpacing: '0.08em',
    color: 'rgba(255,255,255,0.78)',
    textTransform: 'uppercase',
  }),
  performanceList: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '10px',
  }),
  performanceItem: css({
    borderRadius: '18px',
    border: '1px solid rgba(255,255,255,0.06)',
    background: 'rgba(255,255,255,0.02)',
    padding: '14px',
    display: 'flex',
    justifyContent: 'space-between',
    gap: '12px',
  }),
  performanceAsset: css({
    fontWeight: 500,
    fontSize: '0.95rem',
  }),
  performanceMeta: css({
    fontSize: '0.8rem',
    color: 'rgba(255,255,255,0.55)',
  }),
  performanceMetric: css({
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'flex-end',
    gap: '6px',
  }),
  performanceBadge: css({
    borderRadius: '999px',
    border: '1px solid rgba(255,255,255,0.2)',
    paddingInline: '12px',
    paddingBlock: '2px',
    fontSize: '0.7rem',
    letterSpacing: '0.12em',
    textTransform: 'uppercase',
  }),
  performanceValue: css({
    fontSize: '1.2rem',
    fontWeight: 600,
  }),
  filtersCard: css({
    borderRadius: '22px',
    background: 'rgba(255,255,255,0.04)',
    border: '1px solid rgba(255,255,255,0.08)',
    padding: '16px',
    display: 'flex',
    flexDirection: { base: 'column', md: 'row' },
    gap: '12px',
    justifyContent: 'space-between',
    alignItems: { base: 'stretch', md: 'center' },
  }),
  brandsCard: css({
    borderRadius: '22px',
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(255,255,255,0.06)',
    padding: '20px',
    display: 'flex',
    flexDirection: 'column',
    gap: '16px',
  }),
  brandsHeader: css({
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: { base: 'flex-start', md: 'center' },
    flexWrap: 'wrap',
    gap: '12px',
  }),
  brandsTitle: css({
    fontSize: '0.85rem',
    textTransform: 'uppercase',
    letterSpacing: '0.2em',
    color: 'rgba(255,255,255,0.65)',
  }),
  brandsSubtitle: css({
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.6)',
    marginTop: '6px',
    maxW: '520px',
    lineHeight: 1.4,
  }),
  brandsContent: css({
    display: 'flex',
    flexDirection: { base: 'column', lg: 'row' },
    gap: '20px',
  }),
  brandList: css({
    flex: 1,
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
    maxH: '280px',
    overflowY: 'auto',
    paddingRight: '6px',
  }),
  brandListPlaceholder: css({
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.6)',
  }),
  brandItem: css({
    display: 'flex',
    alignItems: 'center',
    gap: '12px',
    padding: '10px 12px',
    borderRadius: '16px',
    background: 'rgba(255,255,255,0.02)',
    border: '1px solid rgba(255,255,255,0.06)',
  }),
  brandLogo: css({
    width: '48px',
    height: '48px',
    borderRadius: '12px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(255,255,255,0.05)',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    fontSize: '0.8rem',
    letterSpacing: '0.2em',
    overflow: 'hidden',
    '& img': {
      width: '100%',
      height: '100%',
      objectFit: 'contain',
    },
  }),
  brandInfo: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '2px',
    flex: 1,
    minW: 0,
  }),
  brandName: css({
    fontWeight: 500,
    fontSize: '0.95rem',
  }),
  brandMeta: css({
    fontSize: '0.8rem',
    color: 'rgba(255,255,255,0.5)',
    whiteSpace: 'nowrap',
    overflow: 'hidden',
    textOverflow: 'ellipsis',
  }),
  brandEditButton: css({
    borderRadius: '999px',
    border: '1px solid rgba(255,255,255,0.2)',
    background: 'transparent',
    color: '#ffffff',
    fontSize: '0.75rem',
    letterSpacing: '0.12em',
    textTransform: 'uppercase',
    paddingInline: '12px',
    paddingBlock: '4px',
    cursor: 'pointer',
    _hover: {
      borderColor: 'rgba(255,255,255,0.4)',
    },
  }),
  brandForm: css({
    flex: { base: 'unset', lg: '0 0 360px' },
    borderRadius: '18px',
    background: 'rgba(255,255,255,0.02)',
    border: '1px solid rgba(255,255,255,0.05)',
    padding: '16px',
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
  }),
  brandFormTitle: css({
    fontSize: '0.85rem',
    textTransform: 'uppercase',
    letterSpacing: '0.2em',
    color: 'rgba(255,255,255,0.6)',
  }),
  brandFormGrid: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '10px',
  }),
  brandFormInput: css({
    borderRadius: '14px',
    border: '1px solid rgba(255,255,255,0.12)',
    background: 'rgba(0,0,0,0.35)',
    paddingInline: '14px',
    paddingBlock: '10px',
    fontSize: '0.9rem',
    color: '#ffffff',
    outline: 'none',
    transition: 'border-color 0.2s ease, box-shadow 0.2s ease',
    '&::placeholder': {
      color: 'rgba(255,255,255,0.45)',
    },
    _focus: {
      borderColor: 'rgba(255,255,255,0.35)',
      boxShadow: '0 0 0 2px rgba(255,255,255,0.08)',
    },
  }),
  brandFormActions: css({
    display: 'flex',
    gap: '10px',
    flexWrap: 'wrap',
    alignItems: 'center',
    '& > button': {
      borderRadius: '999px',
      border: '1px solid rgba(255,255,255,0.18)',
      background: 'transparent',
      color: '#ffffff',
      paddingInline: '16px',
      paddingBlock: '8px',
      fontSize: '0.8rem',
      letterSpacing: '0.12em',
      textTransform: 'uppercase',
      cursor: 'pointer',
      _disabled: {
        opacity: 0.5,
        cursor: 'not-allowed',
      },
    },
    '& > button:first-of-type': {
      background: 'linear-gradient(120deg, #ffffff 0%, #b3b3b3 100%)',
      color: '#050505',
      border: 'none',
    },
  }),
  brandFeedbackError: css({
    fontSize: '0.8rem',
    color: '#ff8080',
  }),
  brandFeedbackSuccess: css({
    fontSize: '0.8rem',
    color: '#7dffb2',
  }),
  usersCard: css({
    borderRadius: '22px',
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(255,255,255,0.06)',
    padding: '20px',
    display: 'flex',
    flexDirection: 'column',
    gap: '16px',
  }),
  userDetailGrid: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(2, minmax(0, 1fr))', md: 'repeat(3, minmax(0, 1fr))' },
    gap: '12px',
  }),
  userDetailCard: css({
    borderRadius: '16px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(0,0,0,0.35)',
    padding: '14px',
    display: 'flex',
    flexDirection: 'column',
    gap: '4px',
  }),
  userDetailSection: css({
    marginTop: '18px',
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
  }),
  userDetailTableWrapper: css({
    width: '100%',
    overflowX: 'auto',
  }),
  userActivityList: css({
    listStyle: 'none',
    margin: 0,
    padding: 0,
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
  }),
  userActivityItem: css({
    borderRadius: '16px',
    border: '1px solid rgba(255,255,255,0.06)',
    background: 'rgba(255,255,255,0.02)',
    padding: '12px',
    display: 'flex',
    justifyContent: 'space-between',
    gap: '12px',
    flexWrap: 'wrap',
  }),
  userActivityType: css({
    fontSize: '0.85rem',
    letterSpacing: '0.08em',
    textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.7)',
  }),
  userActivityDescription: css({
    fontSize: '0.95rem',
    color: '#f7f7f7',
  }),
  userActivityDate: css({
    fontSize: '0.8rem',
    color: 'rgba(255,255,255,0.6)',
    whiteSpace: 'nowrap',
  }),
  infoAlert: css({
    borderRadius: '14px',
    border: '1px solid rgba(255,255,255,0.15)',
    background: 'rgba(255,255,255,0.05)',
    padding: '12px 16px',
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.85)',
  }),
  usersPlaceholder: css({
    fontSize: '0.95rem',
    color: 'rgba(255,255,255,0.65)',
    padding: '12px 0',
  }),
  userStatsRow: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(2, minmax(0, 1fr))', md: 'repeat(4, minmax(0, 1fr))' },
    gap: '12px',
  }),
  userStatCard: css({
    borderRadius: '16px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(255,255,255,0.02)',
    padding: '12px',
    display: 'flex',
    flexDirection: 'column',
    gap: '4px',
  }),
  userStatValue: css({
    fontSize: '1.4rem',
    fontWeight: 600,
  }),
  userStatLabel: css({
    fontSize: '0.75rem',
    letterSpacing: '0.14em',
    textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.6)',
  }),
  userList: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
  }),
  userCard: css({
    borderRadius: '20px',
    border: '1px solid rgba(255,255,255,0.06)',
    background: 'rgba(255,255,255,0.02)',
    padding: '16px',
    display: 'flex',
    flexDirection: 'column',
    gap: '12px',
  }),
  userIdentity: css({
    display: 'flex',
    alignItems: 'flex-start',
    gap: '12px',
  }),
  userAvatar: css({
    width: '48px',
    height: '48px',
    borderRadius: '16px',
    border: '1px solid rgba(255,255,255,0.12)',
    background: 'rgba(255,255,255,0.08)',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    fontWeight: 600,
    fontSize: '1rem',
    letterSpacing: '0.1em',
  }),
  userDetails: css({
    flex: 1,
    minW: 0,
    display: 'flex',
    flexDirection: 'column',
    gap: '2px',
  }),
  userName: css({
    fontSize: '1rem',
    fontWeight: 600,
  }),
  userEmail: css({
    fontSize: '0.85rem',
    color: 'rgba(255,255,255,0.7)',
    overflow: 'hidden',
    textOverflow: 'ellipsis',
  }),
  userMeta: css({
    fontSize: '0.8rem',
    color: 'rgba(255,255,255,0.55)',
  }),
  userStatusBadge: css({
    borderRadius: '999px',
    paddingInline: '12px',
    paddingBlock: '4px',
    fontSize: '0.75rem',
    letterSpacing: '0.12em',
    textTransform: 'uppercase',
    border: '1px solid rgba(255,255,255,0.18)',
  }),
  userStatusActive: css({
    borderColor: 'rgba(0,255,148,0.35)',
    color: '#00ff94',
  }),
  userStatusInactive: css({
    borderColor: 'rgba(255,100,100,0.35)',
    color: '#ff8080',
  }),
  userRoleBadge: css({
    borderRadius: '10px',
    border: '1px solid rgba(255,255,255,0.18)',
    paddingInline: '10px',
    paddingBlock: '4px',
    fontSize: '0.75rem',
    letterSpacing: '0.08em',
    textTransform: 'uppercase',
  }),
  userControls: css({
    display: 'flex',
    flexDirection: { base: 'column', sm: 'row' },
    gap: '10px',
    alignItems: { base: 'stretch', sm: 'center' },
  }),
  userControlButton: css({
    borderRadius: '14px',
    border: '1px solid rgba(255,255,255,0.2)',
    background: 'transparent',
    color: '#ffffff',
    paddingInline: '16px',
    paddingBlock: '10px',
    fontSize: '0.8rem',
    letterSpacing: '0.12em',
    textTransform: 'uppercase',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease, color 0.2s ease, transform 0.2s ease',
    _hover: {
      borderColor: 'rgba(255,255,255,0.4)',
      transform: 'translateY(-1px)',
    },
    _disabled: {
      opacity: 0.5,
      cursor: 'not-allowed',
      transform: 'none',
    },
  }),
  userRefreshButton: css({
    borderRadius: '999px',
    border: '1px solid rgba(255,255,255,0.2)',
    background: 'transparent',
    color: '#ffffff',
    fontSize: '0.75rem',
    letterSpacing: '0.16em',
    textTransform: 'uppercase',
    paddingInline: '16px',
    paddingBlock: '6px',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease',
    _hover: {
      borderColor: 'rgba(255,255,255,0.4)',
    },
    _disabled: {
      opacity: 0.5,
      cursor: 'not-allowed',
    },
  }),
  filtersRow: css({
    display: 'flex',
    gap: '10px',
    flexWrap: 'wrap',
    alignItems: 'center',
  }),
  filterInput: css({
    flex: 1,
    minW: { base: '100%', md: '220px' },
    borderRadius: '14px',
    border: '1px solid rgba(255,255,255,0.12)',
    background: 'rgba(0,0,0,0.35)',
    paddingInline: '14px',
    paddingBlock: '10px',
    fontSize: '0.9rem',
    color: '#ffffff',
    outline: 'none',
    transition: 'border-color 0.2s ease, box-shadow 0.2s ease',
    '&::placeholder': {
      color: 'rgba(255,255,255,0.45)',
    },
    _focus: {
      borderColor: 'rgba(255,255,255,0.35)',
      boxShadow: '0 0 0 2px rgba(255,255,255,0.08)',
    },
  }),
  filterSelectWrapper: css({
    minW: '150px',
    maxW: '200px',
  }),
  tableWrapper: css({
    borderRadius: '26px',
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(255,255,255,0.06)',
    padding: '16px',
    display: 'flex',
    flexDirection: 'column',
    gap: '14px',
  }),
  tableHeader: css({
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
    flexWrap: 'wrap',
    gap: '10px',
  }),
  tableTitle: css({
    fontSize: '1rem',
    letterSpacing: '0.2em',
    textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.6)',
  }),
  tableScrollArea: css({
    width: '100%',
    maxH: '460px',
    overflowY: 'auto',
    overflowX: 'hidden',
    scrollbarWidth: 'none',
    '&::-webkit-scrollbar': {
      display: 'none',
    },
  }),
  table: css({
    width: '100%',
    borderCollapse: 'collapse',
    minW: '720px',
    '& td': {
      verticalAlign: 'middle',
    },
  }),
  theadCell: css({
    textAlign: 'left',
    fontSize: '0.75rem',
    textTransform: 'uppercase',
    letterSpacing: '0.2em',
    color: 'rgba(255,255,255,0.5)',
    paddingBlock: '10px',
    borderBottom: '1px solid rgba(255,255,255,0.08)',
  }),
  row: css({
    borderBottom: '1px solid rgba(255,255,255,0.05)',
    transition: 'background 0.2s ease',
    '&:hover': {
      background: 'rgba(255,255,255,0.02)',
    },
    '& td': {
      verticalAlign: 'middle !important',
    },
  }),
  rowActive: css({
    // background: 'rgba(255,255,255,0.03)',
  }),
  cell: css({
    paddingBlock: '14px',
    paddingRight: '12px',
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.9)',
    verticalAlign: 'middle',
  }),
  assetCell: css({
    display: 'flex',
    alignItems: 'center',
    gap: '14px',
    verticalAlign: 'middle',
  }),
  assetImageWrapper: css({
    width: '64px',
    height: '64px',
    borderRadius: '18px',
    overflow: 'hidden',
    flexShrink: 0,
    background: 'rgba(255,255,255,0.04)',
    border: '1px solid rgba(255,255,255,0.08)',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
  }),
  assetImage: css({
    width: '100%',
    height: '100%',
    objectFit: 'cover',
  }),
  assetPlaceholder: css({
    fontSize: '0.65rem',
    letterSpacing: '0.35em',
    textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.55)',
  }),
  assetInfo: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '4px',
    minW: 0,
  }),
  userNameButton: css({
    border: 'none',
    background: 'transparent',
    color: '#f7f7f7',
    fontSize: '1rem',
    fontWeight: 500,
    textAlign: 'left',
    padding: 0,
    cursor: 'pointer',
    textDecoration: 'underline',
    textDecorationColor: 'rgba(255,255,255,0.2)',
    _hover: {
      textDecorationColor: '#ffffff',
    },
  }),
  title: css({
    fontWeight: 500,
    fontSize: '1rem',
    letterSpacing: '-0.01em',
  }),
  subtitle: css({
    fontSize: '0.75rem',
    color: 'rgba(255,255,255,0.55)',
  }),
  statusBadge: css({
    borderRadius: '999px',
    paddingInline: '12px',
    paddingBlock: '4px',
    fontSize: '0.75rem',
    textTransform: 'uppercase',
    letterSpacing: '0.12em',
    display: 'inline-flex',
    alignItems: 'center',
    gap: '6px',
  }),
  statusDraft: css({
    border: '1px solid rgba(255,255,255,0.2)',
    color: 'rgba(255,255,255,0.7)',
  }),
  statusPending: css({
    border: '1px solid rgba(255,215,0,0.4)',
    color: 'rgba(255,215,0,0.9)',
  }),
  statusPublished: css({
    border: '1px solid rgba(0,255,148,0.35)',
    color: '#00ff94',
  }),
  statusSold: css({
    border: '1px solid rgba(0,122,255,0.35)',
    color: '#5cb6ff',
  }),
  statusRejected: css({
    border: '1px solid rgba(255,100,100,0.35)',
    color: '#ff8080',
  }),
  statusArchived: css({
    border: '1px solid rgba(255,255,255,0.2)',
    color: 'rgba(255,255,255,0.6)',
  }),
  statusUpcoming: css({
    border: '1px solid rgba(255,180,60,0.5)',
    background: 'rgba(255,180,60,0.12)',
    color: '#ffb43c',
    textTransform: 'none',
    letterSpacing: '0.02em',
    fontSize: '0.7rem',
    fontWeight: 500,
  }),
  progressWrapper: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '6px',
    justifyContent: 'center',
    alignItems: 'stretch',
    width: '100%',
    height: '100%',
    '& > span:first-of-type': {
      fontSize: '0.85rem',
      color: 'rgba(255,255,255,0.8)',
      whiteSpace: 'nowrap',
    },
  }),
  progressTrack: css({
    width: '100%',
    minWidth: '120px',
    height: '6px',
    borderRadius: '999px',
    background: 'rgba(255,255,255,0.1)',
    overflow: 'hidden',
  }),
  progressFill: css({
    height: '100%',
    borderRadius: '999px',
    background: 'linear-gradient(90deg, #00ffa3, #00d4ff)',
    transition: 'width 0.3s ease',
  }),
  manageButton: css({
    borderRadius: '999px',
    border: '1px solid rgba(255,255,255,0.2)',
    background: 'transparent',
    color: '#ffffff',
    paddingInline: '14px',
    paddingBlock: '6px',
    fontSize: '0.8rem',
    letterSpacing: '0.12em',
    textTransform: 'uppercase',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease, color 0.2s ease',
    _hover: {
      borderColor: 'rgba(255,255,255,0.4)',
      color: '#fdfdfd',
    },
  }),
  paginationRow: css({
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
    flexWrap: 'wrap',
    gap: '10px',
    fontSize: '0.85rem',
    color: 'rgba(255,255,255,0.6)',
  }),
  paginationButtons: css({
    display: 'flex',
    gap: '8px',
  }),
  paginationButton: css({
    borderRadius: '12px',
    border: '1px solid rgba(255,255,255,0.18)',
    background: 'transparent',
    color: '#ffffff',
    paddingInline: '12px',
    paddingBlock: '6px',
    fontSize: '0.8rem',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease',
    _disabled: {
      opacity: 0.4,
      cursor: 'not-allowed',
    },
  }),
  detailPanelOverlay: css({
    position: 'fixed',
    inset: 0,
    background: 'rgba(0,0,0,0.6)',
    backdropFilter: 'blur(4px)',
    zIndex: 99,
  }),
  detailPanel: css({
    position: 'fixed',
    bottom: 0,
    left: 0,
    right: 0,
    borderRadius: '32px 32px 0 0',
    background: 'rgba(4,4,4,0.98)',
    backdropFilter: 'blur(20px)',
    border: '1px solid rgba(255,255,255,0.08)',
    borderBottom: 'none',
    padding: '24px',
    display: 'flex',
    flexDirection: 'column',
    gap: '16px',
    maxHeight: '80vh',
    overflowY: 'auto',
    zIndex: 100,
    boxShadow: '0 -10px 40px rgba(0,0,0,0.5)',
  }),
  detailHeader: css({
    display: 'flex',
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'flex-start',
    gap: '12px',
  }),
  detailTitle: css({
    fontSize: '1.5rem',
    fontWeight: 600,
  }),
  closeButton: css({
    borderRadius: '50%',
    width: '32px',
    height: '32px',
    border: '1px solid rgba(255,255,255,0.2)',
    background: 'transparent',
    color: '#ffffff',
    fontSize: '1.2rem',
    cursor: 'pointer',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    transition: 'border-color 0.2s ease, background 0.2s ease',
    flexShrink: 0,
    _hover: {
      borderColor: 'rgba(255,255,255,0.4)',
      background: 'rgba(255,255,255,0.05)',
    },
  }),
  primaryActionsSection: css({
    padding: '20px',
    borderRadius: '16px',
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(255,255,255,0.08)',
  }),
  primaryActions: css({
    display: 'flex',
    gap: '12px',
    flexWrap: 'wrap',
  }),
  publishButton: css({
    flex: 1,
    minWidth: '140px',
    borderRadius: '12px',
    border: '1px solid rgba(255,255,255,0.15)',
    background: 'rgba(255,255,255,0.9)',
    color: '#1a1a1a',
    fontSize: '0.95rem',
    fontWeight: 600,
    paddingInline: '20px',
    paddingBlock: '12px',
    cursor: 'pointer',
    transition: 'transform 0.2s ease, box-shadow 0.2s ease, background 0.3s ease',
    position: 'relative',
    _hover: {
      transform: 'translateY(-2px)',
      boxShadow: '0 8px 24px rgba(101,101,101,0.2)',
    },
    _disabled: {
      opacity: 0.5,
      cursor: 'not-allowed',
      transform: 'none',
    },
  }),
  rejectButton: css({
    flex: 1,
    minWidth: '140px',
    borderRadius: '12px',
    border: '1px solid rgba(255,100,100,0.5)',
    background: 'rgba(255,100,100,0.1)',
    color: '#ff8080',
    fontSize: '0.95rem',
    fontWeight: 600,
    paddingInline: '20px',
    paddingBlock: '12px',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease, background 0.2s ease, transform 0.2s ease',
    _hover: {
      borderColor: 'rgba(255,100,100,0.7)',
      background: 'rgba(255,100,100,0.15)',
      transform: 'translateY(-2px)',
    },
    _disabled: {
      opacity: 0.5,
      cursor: 'not-allowed',
      transform: 'none',
    },
  }),
  secondaryActions: css({
    display: 'flex',
    gap: '8px',
    flexWrap: 'wrap',
    paddingTop: '8px',
  }),
  deleteButton: css({
    borderRadius: '12px',
    border: '1px solid rgba(255,100,100,0.3)',
    background: 'transparent',
    color: '#ff8080',
    fontSize: '0.9rem',
    fontWeight: 500,
    paddingInline: '16px',
    paddingBlock: '10px',
    cursor: 'pointer',
    transition: 'border-color 0.2s ease, background 0.2s ease',
    _hover: {
      borderColor: 'rgba(255,100,100,0.5)',
      background: 'rgba(255,100,100,0.1)',
    },
  }),
  detailActions: css({
    display: 'flex',
    gap: '8px',
    flexWrap: 'wrap',
  }),
  detailSection: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '8px',
  }),
  detailSectionTitle: css({
    fontSize: '0.85rem',
    textTransform: 'uppercase',
    letterSpacing: '0.18em',
    color: 'rgba(255,255,255,0.55)',
  }),
  detailGrid: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(1, minmax(0, 1fr))', md: 'repeat(3, minmax(0, 1fr))' },
    gap: '12px',
  }),
  detailCard: css({
    borderRadius: '16px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(0,0,0,0.35)',
    padding: '12px',
    display: 'flex',
    flexDirection: 'column',
    gap: '4px',
  }),
  detailLabel: css({
    fontSize: '0.75rem',
    color: 'rgba(255,255,255,0.6)',
  }),
  detailValue: css({
    fontSize: '1rem',
    fontWeight: 500,
  }),
  investorsPanel: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '10px',
  }),
  investorList: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '8px',
  }),
  investorChip: css({
    borderRadius: '20px',
    border: '1px solid rgba(255,255,255,0.08)',
    padding: '10px',
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
    background: 'rgba(255,255,255,0.02)',
  }),
  investorInfo: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '2px',
  }),
  investorName: css({
    fontWeight: 500,
  }),
  investorMeta: css({
    fontSize: '0.75rem',
    color: 'rgba(255,255,255,0.55)',
  }),
  modalOverlay: css({
    position: 'fixed',
    inset: 0,
    background: 'rgba(0,0,0,0.7)',
    backdropFilter: 'blur(10px)',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    padding: { base: '16px', md: '24px' },
    zIndex: 100,
  }),
  modalContent: css({
    width: '100%',
    maxW: '720px',
    borderRadius: '24px',
    border: '1px solid rgba(255,255,255,0.08)',
    background: 'rgba(6,6,6,0.96)',
    padding: { base: '18px', md: '24px' },
    display: 'flex',
    flexDirection: 'column',
    gap: '16px',
  }),
  modalHeader: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '6px',
  }),
  modalTitle: css({
    fontSize: '1.4rem',
    fontWeight: 600,
  }),
  modalSubtitle: css({
    fontSize: '0.9rem',
    color: 'rgba(255,255,255,0.55)',
  }),
  modalForm: css({
    display: 'grid',
    gridTemplateColumns: { base: 'repeat(1, minmax(0, 1fr))', md: 'repeat(2, minmax(0, 1fr))' },
    gap: '14px',
  }),
  formControl: css({
    display: 'flex',
    flexDirection: 'column',
    gap: '6px',
  }),
  formLabel: css({
    fontSize: '0.8rem',
    textTransform: 'uppercase',
    letterSpacing: '0.18em',
    color: 'rgba(255,255,255,0.55)',
  }),
  input: css({
    borderRadius: '12px',
    border: '1px solid rgba(255,255,255,0.12)',
    background: 'rgba(3,3,3,0.85)',
    paddingInline: '12px',
    paddingBlock: '10px',
    fontSize: '0.9rem',
    color: '#ffffff',
    outline: 'none',
    transition: 'border-color 0.2s ease',
    _focus: {
      borderColor: 'rgba(255,255,255,0.35)',
    },
  }),
  textarea: css({
    borderRadius: '12px',
    border: '1px solid rgba(255,255,255,0.12)',
    background: 'rgba(3,3,3,0.85)',
    padding: '12px',
    minH: '110px',
    fontSize: '0.9rem',
    color: '#ffffff',
    outline: 'none',
    resize: 'vertical',
    transition: 'border-color 0.2s ease',
    _focus: {
      borderColor: 'rgba(255,255,255,0.35)',
    },
  }),
  modalActions: css({
    display: 'flex',
    justifyContent: 'flex-end',
    gap: '10px',
  }),
  deleteWarning: css({
    padding: '16px',
    borderRadius: '12px',
    background: 'rgba(255,100,100,0.1)',
    border: '1px solid rgba(255,100,100,0.3)',
    display: 'flex',
    flexDirection: 'column',
    gap: '8px',
  }),
  deleteWarningText: css({
    fontSize: '0.95rem',
    fontWeight: 600,
    color: '#ff8080',
    lineHeight: 1.5,
  }),
  deleteWarningSubtext: css({
    fontSize: '0.85rem',
    color: 'rgba(255,255,255,0.7)',
    lineHeight: 1.4,
  }),
  deleteConfirmButton: css({
    borderRadius: '12px',
    border: 'none',
    background: 'linear-gradient(135deg, #ff6b6b 0%, #ff5252 100%)',
    color: '#ffffff',
    fontSize: '0.9rem',
    fontWeight: 600,
    paddingInline: '20px',
    paddingBlock: '10px',
    cursor: 'pointer',
    transition: 'transform 0.2s ease, box-shadow 0.2s ease',
    _hover: {
      transform: 'translateY(-2px)',
      boxShadow: '0 8px 24px rgba(255,107,107,0.4)',
    },
    _disabled: {
      opacity: 0.5,
      cursor: 'not-allowed',
      transform: 'none',
    },
  }),
  errorText: css({
    color: '#ff8080',
    fontSize: '0.85rem',
  }),
};

